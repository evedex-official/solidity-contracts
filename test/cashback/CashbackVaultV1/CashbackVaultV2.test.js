...
describe('CashbackVaultV1', function () {
  ...
  before(async function () {
    ...
    const CashbackVaultV1 = await ethers.getContractFactory('contracts/cashback/CashbackVaultV2.sol:CashbackVaultV2');
  ...
  it('should update signer', async () => {
    const newSignerAddress = notOwner.address;
    const originalSigner = await cashbackVault.signer();
    await cashbackVault.connect(owner).setSigner(newSignerAddress);
    const updatedSigner = await cashbackVault.signer();
    expect(updatedSigner).to.eq(newSignerAddress);
    await cashbackVault.connect(owner).setSigner(originalSigner);
    expect(await cashbackVault.signer()).to.eq(originalSigner);
    await expect(cashbackVault.connect(owner).setSigner(ethers.ZeroAddress)).to.be.revertedWithCustomError(
      cashbackVault,
      'InvalidSigner',
    );
    await expect(cashbackVault.connect(notOwner).setSigner(newSignerAddress)).to.be.revertedWithCustomError(
      cashbackVault,
