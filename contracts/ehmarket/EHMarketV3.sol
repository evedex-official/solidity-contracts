...
contract EHMarketV3 is AccessControlEnumerableUpgradeable {
    ...
    event BlockRequestId(uint256 requestId);
    event UnblockRequestId(uint256 requestId);
    ...
    // 0 - free
    // 1 - used
    // 2 - blocked by matcher
    // 3 - blocked by admin
    mapping(uint256 => uint8) public requestIds;
    ...
    function withdrawAsset(address from, uint256 amount, uint256 requestId) external {
        ...
        uint8 reqId = requestIds[requestId];
        if (reqId == 1) revert RequestIdAlreadyUsed();
        if (reqId >= 2) revert RequestIdBlocked();
    ...
    function blockRequestId(uint256 requestId) public {
        bool isMatcher = hasRole(MATCHER_ROLE, _msgSender());
        bool isAdmin = hasRole(DEFAULT_ADMIN_ROLE, _msgSender());
        require(isMatcher || isAdmin, UnauthorizedSigner());
        if (requestIds[requestId] != 0) revert InvalidRequestId();
        if (isAdmin) requestIds[requestId] = 3;
        else requestIds[requestId] = 2;
        emit BlockRequestId(requestId);
    }

    function unblockRequestId(uint256 requestId) public {
        bool isMatcher = hasRole(MATCHER_ROLE, _msgSender());
        bool isAdmin = hasRole(DEFAULT_ADMIN_ROLE, _msgSender());
        require(isMatcher || isAdmin, UnauthorizedSigner());
        uint8 reqIdStatus = requestIds[requestId];
        if (reqIdStatus < 2) revert InvalidRequestId();
        if (reqIdStatus == 3 && !isAdmin) revert UnauthorizedSigner();
        delete requestIds[requestId];
        emit UnblockRequestId(requestId);
    }
