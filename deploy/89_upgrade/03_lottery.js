...
module.exports = migration(async (deployer) => {
  await deployer.deployProxyImplementation('Lottery', 'contracts/lottery/LotteryV2.sol:LotteryV2', {
    unsafeAllow: ['constructor'],
  });

  const proxy = await deployer.getContract('Lottery');
  const proxyAddress = await proxy.getAddress();
  const artifact = await hardhat.artifacts.readArtifact('contracts/lottery/LotteryV2.sol:LotteryV2');
...
module.exports.tags = ['Upgradable', 'UpgradeLottery'];
